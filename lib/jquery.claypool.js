var Claypool={
/**
 * Claypool jquery.claypool.1.1.pre04 - A Web 1.6180339... Javascript Application Framework
 *
...
(function($, $$, $M, $Web){
    ...
    $.extend($Web.RestServlet.prototype, 
        ...
        handleGet: function(event, response){
            ...
            if(!domain && !id){
                ...
                this.db.get({
                    async: false,
                    success: function(result){
                        handleSuccess(event, result, response, _this);
                    },
                ...
                log.debug("Handling GET for %s %s", domain, event.params);
                for(var param in event.request.parameters){
                    log.debug('param[%s]=%s', param, event.request.parameters[param]);
                }
                if(event.request.parameters&&('id' in event.request.parameters)){
                    log.debug("LIST OF ITEMS!!!");
                    //response is batch get of items by id
                    ids = event.request.parameters.id.split(',');
        ...
        handlePost: function(event, response){
            ...
            log.debug("Handling POST for %s %s", domain, id).
                debug("Reading POST body %s", event.request.body);
    ...
    var handleSuccess = function(event, result, response, servlet){
        response.headers.status = 200;
        response.headers['Content-Type'] = 'text/javascript';
        response.body = 
            servlet.js2json($.extend(result, event.params()), null, '\t');
    };
